...
async def analyze(user_id: str, meal_type: str, date: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    # Створюємо запис з прив'язкою до обраної дати
    dt = datetime.strptime(date, '%Y-%m-%d')
    new_food = FoodLog(user_id=str(user_id), meal_type=meal_type, food_name=d['name'], calories=d['kcal'], protein=d['p'], fat=d['f'], carbs=d['c'], sugar=d['sugar'], salt=d['salt'], created_at=dt)
    db.add(new_food)
    db.commit()
    return d
...
async def get_stats(user_id: str, date: str, db: Session = Depends(get_db)):
    ...
    for f in food:
        if f.meal_type in meals: meals[f.meal_type] += f.calories
