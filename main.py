...
class FoodLog(Base):
    ...
    sugar = Column(Float, default=0, nullable=True); salt = Column(Float, default=0, nullable=True)
...
async def analyze(text_input: str = Form(None), file: UploadFile = File(None)):
    ...
    txt = res.text[res.text.find("{"):res.text.rfind("}")+1]
...
async def get_stats(user_id: str, date: str, db: Session = Depends(get_db)):
    ...
    # Пріоритет: ручні норми -> розрахунок -> дефолт
    norms = {"kcal": 2000, "p": 120, "f": 70, "c": 250, "sugar": 50, "salt": 5}
    if p:
        if p.c_kcal:
            norms = {"kcal": p.c_kcal, "p": p.c_p or 120, "f": p.c_f or 70, "c": p.c_c or 250, "sugar": 50, "salt": 5}
        else:
            val = (10 * p.weight) + (6.25 * p.height) - (5 * p.age) + (5 if p.gender == 'male' else -161)
            norms["kcal"] = int(val * 1.2)
    ...
    for mt in ["Breakfast", "Lunch", "Dinner", "Snack"]:
        ...
        # ФІКС NaN: додаємо 'or 0' для кожного поля
        meals_data[mt] = {
    ...
    return {
        ...
        "norms": norms, "meals": meals_data, "has_profile": p is not None
    }
...
async def confirm_save(data: dict, db: Session = Depends(get_db)):
    db.add(FoodLog(user_id=str(data['user_id']), meal_type=data['meal_type'], food_name=data['name'], 
                   ...
                   carbs=float(data['c']), sugar=float(data.get('sugar',0)), salt=float(data.get('salt',0)), 
...
async def save_profile(data: dict, db: Session = Depends(get_db)):
    db.merge(UserProfile(user_id=str(data['user_id']), gender=data['gender'], weight=float(data['weight']), 
                         height=float(data['height']), age=int(data['age'] or 25), goal=data['goal'],
