...
# Шлях до бази (з підтримкою Volume на Railway)
DB_PATH = "/app/data/fitlio.db" if os.path.exists("/app/data") else "./fitlio.db"
...
class FoodLog(Base):
    ...
    sugar = Column(Float, default=0.0); salt = Column(Float, default=0.0) # ФІКС
...
# АВТО-МІГРАЦІЯ (Додає колонки, якщо база стара)
...
async def get_stats(user_id: str, date: str, db: Session = Depends(get_db)):
    ...
    norms = {"kcal": p.c_kcal or 2000, "p": p.c_p or 120, "f": p.c_f or 70, "c": p.c_c or 250, "sugar": 50, "salt": 5} if p else {"kcal": 2000, "p": 120, "f": 70, "c": 250, "sugar": 50, "salt": 5}
    ...
    for mt in ["Breakfast", "Lunch", "Dinner"]:
        ...
        meals_data[mt] = {
            ...
            "sugar": round(sum(getattr(i, 'sugar', 0) or 0 for i in items), 1),
            "salt": round(sum(getattr(i, 'salt', 0) or 0 for i in items), 1),
    ...
    return {
        ...
        "sugar": sum(getattr(f, 'sugar', 0) or 0 for f in food), 
        "salt": sum(getattr(f, 'salt', 0) or 0 for f in food),
        "water": round(sum(w.amount or 0 for w in water), 2),
        "norms": norms, "meals": meals_data
    }
...
async def save_profile(data: dict, db: Session = Depends(get_db)):
    db.merge(UserProfile(user_id=str(data['user_id']), weight=float(data['weight']), 
                         height=float(data['height']), goal=data['goal'],
