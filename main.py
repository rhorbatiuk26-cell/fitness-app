...
# Функція для вибору робочої моделі
def get_working_model():
    try:
        # Пробуємо знайти flash модель серед доступних
        for m in genai.list_models():
            if 'gemini-1.5-flash' in m.name and 'generateContent' in m.supported_generation_methods:
                return genai.GenerativeModel(m.name)
        # Якщо не знайшли за списком, пробуємо стандартну
...
@app.get("/")
async def health():
    return {"status": "v34.3 Auto-Model Active"}
...
async def chat(user_id: str, message: str, db: Session = Depends(get_db)):
    try:
        ...
        # Використовуємо ту саму перевірену модель
        response = model.generate_content(prompt)
        return {"reply": response.text}
    except Exception as e:
        # Якщо впало — пробуємо ще раз з прямою назвою
        try:
            alt_model = genai.GenerativeModel('models/gemini-1.5-flash')
            res = alt_model.generate_content(f"Відповідай коротко: {message}")
            return {"reply": res.text}
        except:
            return {"reply": f"Помилка конфігурації API: {str(e)}"}
